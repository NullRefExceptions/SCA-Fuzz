...
/*     RTN rtn = INS_Rtn(ins);
    if (RTN_Valid(rtn))
      LOGE("[uninstrumented] %s at 0x%lx in %s of %s\n", INS_Disassemble(ins).c_str(),INS_Address(ins),RTN_Name(rtn).c_str(),
      IMG_Name(SEC_Img(RTN_Sec(rtn))).c_str());
    else
      LOGE("[uninstrumented] %s at 0x%lx in unknown\n", INS_Disassemble(ins).c_str(),INS_Address(ins)); */
