...
/*     LOGD("combine\n");
    ...
    //申请ssa存放新产生的tag
    //返回的bdd可能已经之前返回过，存在多个ssa_tag保存了这个BDD，此时不影响安全性
    //因为BDD package内部也有引用计数器，多个ssa_tag相当于多次调用sylvan_protect，
    //多个ssa_tag引用为零时，gc过程中调用多次sylvan_unprotect
    uint64_t ssa_idx;
    do
    {
        ssa_idx = free_ssa._h;
...
void ssa_tag2set(ssa_tag const &tag, set<uint32_t> &offset_buf)
{
    if (tag.ssa_ref != NULL)
    {
        uint8_t res[TAG_WIDTH];
